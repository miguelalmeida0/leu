...
public struct GeneralClaimExtractor: Sendable {
    ...
    public func extract(_ text: String, evidence: SemanticEvidenceSpan) -> [ExtractedClaim] {
        // Preserve the role of an entire labelled memory paragraph before splitting
        // its clauses. A later sentence must not lose the leading MEMORY label.
        guard !InstructionalText.isMemoryContent(text) else { return [] }
