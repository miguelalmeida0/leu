...
public struct LearningCandidateValidator: Sendable {
    ...
    public func validate(_ c: LearningModelCandidate, packet: LearningSourcePacket,
        ...
        // V2 independently proves the complete canonical claim above; this lexical concept
        // check permits only PDF whitespace equivalence. Legacy matching stays unchanged.
        let conceptSource = c.selection == nil ? packet.sourceText : CanonicalWhitespaceResolver.normalize(packet.sourceText)
        guard c.prompt.count >= 30, c.prompt.count <= 240, c.prompt.hasSuffix("?"),
              !c.prompt.localizedCaseInsensitiveContains(answer),
              !c.concept.isEmpty, conceptSource.localizedCaseInsensitiveContains(c.concept),
        ...
        // V2 has already proved exact agreement with an independently recompiled claim.
    ...
    public enum Rejection: String, Error, Codable, Sendable {
        ...
        case noRepresentableQuestion, claimMismatch
    }
