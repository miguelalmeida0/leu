...
/// Dependency-free fallback for Linux tests. Apple builds use CryptoKit in FileDigest.
struct PortableSHA256 {
    ...
    mutating func update(_ data: Data) {
        ...
        if !pending.isEmpty {
            ...
            pending.append(contentsOf: bytes.prefix(needed)); offset += needed
            if pending.count == 64 { compress(pending); pending.removeAll(keepingCapacity: true) }
        }
        while offset + 64 <= bytes.count {
            compress(Array(bytes[offset..<(offset + 64)])); offset += 64
        }
        if offset < bytes.count { pending.append(contentsOf: bytes[offset...]) }
    }
    mutating func finalize() -> String {
        ...
        for shift in stride(from: 56, through: 0, by: -8) {
            pending.append(UInt8(truncatingIfNeeded: bitCount >> shift))
        }
    ...
    private mutating func compress(_ block: [UInt8]) {
        ...
        for i in 0..<64 {
            ...
            let choice = (e & f) ^ (~e & g)
            let t1 = h &+ sum1 &+ choice &+ Self.constants[i] &+ w[i]
        ...
        for i in state.indices { state[i] = state[i] &+ working[i] }
    }
