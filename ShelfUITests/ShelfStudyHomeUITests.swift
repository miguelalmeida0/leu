...
final class ShelfStudyHomeUITests: ShelfUITestCase {
    func testEmptyStudyFieldsAndExistingDestinations() {
        ...
        // At AX1 the duration grid is below the subject rows and is realized on scroll.
        let duration = revealStudyButton("10 minutes")
        XCTAssertTrue(app.buttons["10 minutes"].exists)
        assertClearance(duration)
        capture("empty-builder-duration-reachable")
