...
os.chdir(root)
build = root / '.build/v28'
proof = root / 'docs/v28/evidence'
...
env = dict(os.environ, TMPDIR=str(build), V27_TEST_ROOT=str(build), V28_TEST_ROOT=str(build), DYLD_FRAMEWORK_PATH=str(platform / 'Library/PrivateFrameworks'))
...
files = [root / 'Packages/ShelfCore/Tests/ShelfCoreTests/Learning' / name for name in
    ['V27IntelligenceTests.swift', 'GroundedQuestionContractTests.swift', 'V28PersistenceTests.swift', 'V28SourceRoleTests.swift']]
...
suite.addTest(V28PersistenceTests.defaultTestSuite)
suite.addTest(V28SourceRoleTests.defaultTestSuite)
suite.run()
